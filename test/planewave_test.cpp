...
void compare_planewave_nufft() {
    HPDMKParams params;
    params.n_per_leaf = 5;
    params.eps = 1e-3;
    params.L = 20.0;
    params.nufft_eps = 1e-8;
    params.nufft_threshold = 1;
    ...
    for (int l = 2; l < tree.max_depth - 1; l++) {
        for (auto i_node : tree.level_indices[l]) {
            if (bool(tree.GetNodeAttr()[i_node].Leaf)) {
                continue;
            }
...
void compare_planewave_nudft() {
    HPDMKParams params;
    params.n_per_leaf = 5;
    params.eps = 1e-3;
    params.L = 20.0;
    params.nufft_threshold = 1000;
...
TEST(PlanewaveTest, BasicAssertions) {
    ...
    compare_planewave_nufft();
    compare_planewave_nudft();
