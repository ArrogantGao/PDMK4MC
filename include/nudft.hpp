...
namespace hpdmk {
    ...
    template<typename T>
    void nudft3d1_single_halfplane(const T x, const T y, const T z, const complex<T> c, const int iflag, const int N1, const int N2, const int N3, complex<T>* x_cache, complex<T>* y_cache, complex<T>* z_cache, complex<T>* f){
        T iflag_sign = iflag > 0 ? 1 : -1;
        ...
        auto exp_x0 = exp(complex<T>(0, iflag_sign * x));
        auto exp_y0 = exp(complex<T>(0, iflag_sign * y));
        auto exp_z0 = exp(complex<T>(0, iflag_sign * z));
        
        double kx_min = - N1 / 2;
        double ky_min = - N2 / 2;
        double kz_min = - N3 / 2;

        x_cache[0] = exp(complex<T>(0, iflag_sign * x * (kx_min))) * c;
        y_cache[0] = exp(complex<T>(0, iflag_sign * y * (ky_min)));
        z_cache[0] = exp(complex<T>(0, iflag_sign * z * (kz_min)));
